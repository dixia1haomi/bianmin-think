...
class ShanchuRenzhaoche extends MobanXiaoxi
{
    // 删除人找车信息通知
    ...
    public function sendShanchuRenzhaocheMessage($value)
    {
        ...
        if ($backmsg['errcode'] != 0) {
            ...
            Log::init([
                'type' => 'File',
                'path' => LOG_PATH_CTONTAB_SHANCHURENZHAOCHE,   // 人找车日志文件路径
            ]);
            Log::record('删除人找车信息通知失败，' . $backmsg['errmsg'], 'info');
        }
    }
    ...
    private function createMessageData()
    {
        ...
        $data = [
            ...
            'keyword2' => [
                'value' => '亲，您发布的人找车信息已过出发时间，系统已为你自动删除，感谢您使用袋鼠同城。',
                'color' => '#27408B'
            ]
        ];
        $this->data = $data;
    ...
    private function getOpenID($uid)
    {
        ...
        if ($user === false) {
            throw new QueryDbException(['msg'=>'模板消息删除人找车getOpenID出错']);
        }
        return $user->openid;
    }
}