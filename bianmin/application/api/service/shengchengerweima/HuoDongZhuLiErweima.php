...
class HuoDongZhuLiErweima
{
    ...
    public function getCanShuMa($scene)
    {
        ...
        if ($nda !== false) {
            ...
            throw new ErWeiMaException(['msg' => '向微信请求的二维码返回的二进制流值中包含errcode,HuoDongZhuLiErweima']);
        }

        // 把数据流转成图片保存到本地

        file_put_contents(APP_PUBLIC . "huodongzhuli/" . $scene . ".jpeg", $da);
        // Image类处理图片
        // ***
        $haibao_base = Image::open(APP_PUBLIC . 'erweimamoban/bmxxfenxianghaibao.jpg');

        $haibao_base->water(APP_PUBLIC . "huodongzhuli/" . $scene . ".jpeg", 5)
            ->save(APP_PUBLIC . "huodongzhuli/" . $scene . ".jpeg");

        return '/huodongzhuli/' . $scene . '.jpeg';
    }
}